...
class ListCodecSpec extends CodecSpec {

  private val codec = ListCodec[String]
  ...
  property("valid strings should be correctly parsed") {
    val validStrings = Table[String, List[String]](
      ("string", "value"),
      ("[]", List()),
      ("['']", List("")),
      ("[ 'foo', 'bar' ]", List("foo", "bar")),
      ("", List()), // per convention, collection codecs never return null
      ("NULL", List())
    )
    forAll(validStrings) { (s: String, o: List[String]) =>
      codec.parse(s) should equal(o)
    }
  }

  property("valid values should be correctly formatted") {
    val validValues = Table[List[String], String](
      ("value", "string"),
      (List(), "[]"),
      (List(), "[]"),
      (List(""), "['']"),
    ...
    forAll(validValues) { (o: List[String], s: String) =>
  ...
  property("invalid values should be rejected when serializing") {
    an[NullPointerException] should be thrownBy codec.serialize(List(null))
  }
