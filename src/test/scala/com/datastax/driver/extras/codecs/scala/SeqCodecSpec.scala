...
class SeqCodecSpec extends CodecSpec {

  private val codec = SeqCodec[String]
  ...
  property("valid strings should be correctly parsed") {
    val validStrings = Table[String, Seq[String]](
      ("string", "value"),
      ("[]", Seq()),
      ("['']", List("")),
      ("[ 'foo', 'bar' ]", List("foo", "bar")),
      ("", Seq()), // per convention, collection codecs never return null
      ("NULL", Seq())
    )
    forAll(validStrings) { (s: String, o: Seq[String]) =>
      codec.parse(s) should equal(o)
    }
  }

  property("valid values should be correctly formatted") {
    val validValues = Table[Seq[String], String](
      ("value", "string"),
      (Seq(), "[]"),
      (List(), "[]"),
      (List(""), "['']"),
    ...
    forAll(validValues) { (o: Seq[String], s: String) =>
